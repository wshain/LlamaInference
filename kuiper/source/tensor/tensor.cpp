...
static size_t reduce_dimension(T begin, T end, Tp init) {
  ...
  // 使用 std::accumulate 和 std::multiplies 计算 [begin, end) 区间内所有维度的乘积
  // 即：dim0 * dim1 * dim2 * ... （用于计算张量总大小）
  // init 通常是 1
  // accumulate 从 init 开始，对区间内每个元素做乘法
...
Tensor::Tensor(base::DataType data_type, int32_t dim0, bool need_alloc,
  ...
  dims_.push_back(dim0);  // 设置维度
  size_ = dim0;           // 总元素数 = dim0
  if (need_alloc && alloc) {
    allocate(alloc);  // 需要分配内存， 则调用allocate函数
  } else {
    if (ptr != nullptr) {  // 如果传入外部指针ptr,则不能同时要求内部分配，会冲突
      CHECK(need_alloc == false)  // 检查
          << "The need_alloc is is true when ptr parameter is not a null pointer.";
      init_buffer(alloc, data_type_, need_alloc, ptr);  // 外部指针初始化buffer
    }
  }
}
